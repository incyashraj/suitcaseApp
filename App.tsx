...
import { searchBooks, consultConcierge, getOnboardingRecommendations } from './services/geminiService';
...
export default function App() {
  ...
  return (
    <div 
        className="min-h-screen flex flex-col bg-[#F5F5F7] selection:bg-blue-100 overflow-x-hidden relative"
    ...
    >
      ...
      {/* Drag Overlay */}
      {isDragging && (
          <div className="fixed inset-0 z-[100] bg-blue-500/10 backdrop-blur-sm border-4 border-blue-500 border-dashed m-4 rounded-[3rem] flex items-center justify-center pointer-events-none animate-in fade-in duration-200">
              <div className="bg-white p-8 rounded-3xl shadow-2xl flex flex-col items-center">
                  <Upload size={48} className="text-blue-500 mb-4 animate-bounce"/>
                  <h2 className="text-2xl font-bold text-gray-900">Drop PDF to Upload</h2>
              </div>
          </div>
      )}
      ...
      <div className="fixed top-0 left-0 right-0 z-30 px-6 py-4 flex justify-between items-center pointer-events-none">
          <div className="flex items-center gap-2 mix-blend-multiply opacity-50">
            <Briefcase size={20} className="text-gray-900" />
            <span className="font-serif font-bold text-xl tracking-tight text-gray-900 hidden sm:block">Suitcase</span>
          </div>

          <div className="pointer-events-auto flex items-center gap-3">
             <button onClick={handleLogout} className="bg-white/60 backdrop-blur-md p-2 rounded-full shadow-sm border border-white/50 text-gray-500 hover:text-red-500 transition-colors">
                 <LogOut size={16} />
             </button>
             
             <div 
                onClick={() => { setActiveMode('library'); setView('profile'); }}
                className="bg-white/60 backdrop-blur-md px-4 py-2 rounded-full shadow-sm border border-white/50 flex items-center gap-2 hover:scale-105 transition-transform cursor-pointer"
             >
                <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-yellow-400 to-yellow-600 flex items-center justify-center shadow-sm">
                   <span className="text-[10px] font-bold text-white">$</span>
                </div>
                <span className="text-sm font-bold text-gray-800">{wallet.balance}</span>
             </div>
          </div>
      </div>
                 ...
                 <div className="flex-1 overflow-y-auto space-y-6 pb-24 px-4 scroll-smooth">
                    {conciergeHistory.map((msg, idx) => (
                        <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} animate-in fade-in slide-in-from-bottom-2`}>
                            {/* Message Bubble */}
                            <div className={`max-w-[85%] md:max-w-[70%] p-5 rounded-[1.5rem] text-base leading-relaxed shadow-sm ${
                                msg.role === 'user' 
              ...
              <section>
                ...
                {myLibrary.length === 0 ? (
                   ...
                   >
                     ...
                     <button className="bg-black text-white px-6 py-2 rounded-full text-sm font-bold">Import Book</button>
                   </div>
                ) : (
                   <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-x-6 gap-y-10">
                      ...
                      <button 
                         onClick={() => fileInputRef.current?.click()}
                         className="aspect-[2/3] rounded-2xl bg-gray-100 border-2 border-dashed border-gray-300 hover:border-blue-400 hover:bg-blue-50 transition-all group flex flex-col items-center justify-center gap-3"
                      >
                         ...
                         <span className="text-xs font-bold text-gray-400 group-hover:text-blue-600 uppercase tracking-wide">Add Book</span>
                      </button>
      ...
      {view !== 'reader' && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-40 animate-in slide-in-from-bottom-24 duration-1000 ease-out">
           <div className="glass-panel flex items-center gap-2 p-2 px-4 rounded-full shadow-[0_25px_60px_-15px_rgba(0,0,0,0.15)] ring-1 ring-white/60 bg-white/80 backdrop-blur-xl">
